	...
	$(function() {
		...
		$(function() {
			$('#create').click(function(){
				$.ajax({//데이터 전달
					url: "dyreviewwrite.jsp",//전달할 페이지
					data: {
						user_id: $('#user_id').text(),//전달할 데이터
						product_id: $('#product').val(),
						star: $('#star').val(),
						review_content: $('#review_content').val(),
					},
					success : function(check){//성공시 실행
...
<body>
	...
	<div class="container">
		...
		<tr>			
			...
			<% 
				ProductDAO pdao = new ProductDAO();//product_id로 상품명 찾는 메서드 사용
				for(int i = 0; i < clist.size(); i++){
				CartVO cvo = clist.get(i);//list를 하나씩 뽑아줌
			%>
				<option style="text-align:center;" value="<%=cvo.getProduct_id()%>">
															<!-- 상품id검색 -->
		...
		<tr>
			...
			<td class="col2">
				<div id="user_id" style=" border:0; outlnie:0; text-align: center; display: block; margin: 0 auto" >
		...
		<tr>	
			<td class="col1"><b>별점*</b></td>
	  		<td class="score">
	  			<input id="star" style="width: 300px; display: block; margin: 0 auto; 
	  			text-align:center;" type="text" placeholder="0.5단위로 평가해주세요.ex)4.5"/>
	  		</td>
		</tr>
